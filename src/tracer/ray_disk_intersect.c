...
/*   Updated: 2024/08/09 17:40:23 by Philip           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool ray_disk_intersect(t_ray* ray, t_object* disk, t_closest* closest)
{
    double   denominator;
    t_vector intersect;
    ...
    if (!equals(denominator, 0.0))
    {
        ...
        if (distance_squared <= disk->radius_squared && t >= ray->t_min
            && t <= ray->t_max && t < closest->t)
        {
            closest->t = t;
            closest->object = disk;
