...
/*   Updated: 2024/08/09 17:45:50 by Philip           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

// /* Comment out this line to activate this file.
...
void load_default_scene(t_scene* scene)
{
    ...
    scene->objects[--i] = checkerboard_sphere((t_raw_point){0, -0.02, -2}, 0.33,
                                              10.0, 0.2);
    ...
    // Left wall
    scene->objects[--i] = plane(BLUE, (t_raw_point){-960.0 / 960.0, 0, 0},
                                (t_vector){-1, 0, 0}, 100.0, 0.1);
    // Right wall
    scene->objects[--i] = plane(CYAN, (t_raw_point){960.0 / 960.0, 0, 0},
                                (t_vector){1, 0, 0}, 10.0, 0.0);
    // Ceiling
    scene->objects[--i] = plane(WHITE, (t_raw_point){0, 540.0 / 960.0, 0},
                                (t_vector){0, 1, 0}, 10.0, 0.0);
    // Floor
    scene->objects[--i] = plane(0x808080, (t_raw_point){0, -540.0 / 960.0, 0},
                                (t_vector){0, -1, 0}, 10.0, 0.1);
    // Back wall
    scene->objects[--i] = plane(0x808080, (t_raw_point){0, 0, -3000.0 / 960.0},
                                (t_vector){0, 0, -1}, 10.0, 0.0);
    // Disk mirror
    scene->objects[--i] = disk(
        WHITE, (t_raw_point){700.0 / 960.0, -150.0 / 960.0, -2500.0 / 960.0},
        (t_vector){1, 0, -1}, 300.0 / 960.0, 1000.0, 0.9);
    scene->objects[--i] = cylinder(
        RED, (t_raw_point){10.0 / 960.0, -540.0 / 960.0, -2000.0 / 960.0},
...
void load_test_scene(t_scene* scene)
{
    ...
    // Wall in back
    // scene->objects[--object_count] = plane(WHITE,
    ...
    scene->objects[--object_count] = cylinder(
        RED, (t_point){0 / 960.0, 0 / 960.0, -1500.0 / 960.0},
